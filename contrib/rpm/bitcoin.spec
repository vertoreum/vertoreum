...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16179
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16178
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 26179
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 26178
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 16179
	%{_sbindir}/semanage port -d -p tcp 16178
	%{_sbindir}/semanage port -d -p tcp 26179
	%{_sbindir}/semanage port -d -p tcp 26178
	for selinuxvariant in %{selinux_variants}; do
